...
export async function mediaRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/:assetId/upload-complete', {
    ...
    const asset = await prisma.mediaAsset.findFirst({
      where: { id: request.params.assetId, userId: request.user.sub },
    })
...
export async function projectRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/:projectId', {
    ...
    const project = await prisma.project.findFirst({
      where:   { id: request.params.projectId, userId: request.user.sub },
  ...
  app.get('/:projectId/exports/:exportId', {
    ...
    request: FastifyRequest<{ Params: { projectId: string; exportId: string } }>,
    reply: FastifyReply
  ) => {
    const exportRecord = await prisma.export.findFirst({
      where: { id: request.params.exportId, projectId: request.params.projectId },
    })
