// ============================================================
//  Media Service — Fastify Plugin Registration
// ============================================================
...
export async function registerPlugins(app: FastifyInstance): Promise<void> {
  ...
  // Multipart for any direct uploads (small files only)
  // Large files go via presigned R2 URL directly from mobile
  await app.register(fastifyMultipart, {
    limits: { fileSize: 100 * 1024 * 1024 }, // 100MB cap for direct uploads
  })
  ...
  if (config.NODE_ENV !== 'production') {
    await app.register(fastifySwagger, {
      openapi: {
        openapi: '3.1.0',
        info: { title: 'StreamForge Media API', version: '1.0.0' },
        tags: [
          { name: 'Media',    description: 'Asset upload and management' },
          { name: 'Projects', description: 'Video editing projects' },
        ],
        components: {
          securitySchemes: {
